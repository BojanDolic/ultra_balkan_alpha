...
main()
{
  ...
  print("========================================================");
	...
	print("=======================================================");
	printf("	    VERZIJA MODA: %s",SCRIPT);
	printf("	    ZADNJI UPDATE: %s",UPDATE);
  print("===============================================");
}
//===============================[ NEWS ]=======================================
new MySQL:handle;
...
{
	new name[MAX_PLAYER_NAME],vehid = GetPlayerVehicleID(playerid);
...
task PayDay[60000]() {
  ...
  // Ovo na svakoj plati resetuje svim igračima u bazi payday poene na 0 i automatski se izbjegava zloupotreba
  if(minute == 0) {
...
YCMD:makeadmin(playerid,params[], help)
{
	...
	if(sscanf(params,"iii",id, level, slot))
	{
 		SCM(playerid, SIVA," Koristi: /makeadmin [id] [Level Admina] [slot]");
  ...
  if(slot > 15 || slot < 1) return SendClientMessage(playerid, SIVA, "Slot ne moze biti manji od 1 ili veci od 15");
	if(level < 1 || level > 5) return SendClientMessage(playerid, SIVA, "Admin level ne moze biti veci od 5 ili manji od 1");
  if(PlayerInfo[id][pAdmin] > 0) { // Ako imate admina preko /5 komande ovo nece raditi
    ...
    va_SendClientMessage(id, -1, "[UB Administracija] Cestitamo. Postali ste clan Admin tima | Level: %d | Slot: %d", level, slot);
    va_SendClientMessage(playerid, -1, "Postavili ste igraca [ID:%d] %s za clana Admin tima | Level: %d | Slot: %d", id, GetName(id), level, slot);

    mysql_format(handle, stringSmall, sizeof(stringSmall), "UPDATE admini SET uidAdmina = %d , imeAdmina = '%e' , adminLevel = %d , datum = NOW() WHERE id = %d", PlayerInfo[id][pUID], GetName(id), PlayerInfo[id][pAdmin], slot);
    mysql_tquery(handle, stringSmall);
  }
...
YCMD:clearadmin(playerid, params[], help) 
{
...
YCMD:clearmemberslot(playerid, params[], help) 
{
  new slot, orgID;
...
YCMD:makeleader(playerid, params[], help)
{
  ...
  if(orgid == 0)
    ...
    // UPDATE na tabelu da nebi slučajno kasnije se nešto izj***lo
...
YCMD:clearleaderslot(playerid, params[], help) 
{
  new slot;
...
YCMD:a(playerid, params[], help)
{
	...
	if(PlayerInfo[playerid][pAdmin] < 1) return SCM(playerid, SIVA,"[UB:RP] Niste ovlasceni da koristite ovu komandu");
	if(sscanf(params,"s[150]",string)) return SCM(playerid, SIVA,"Koristi: /a [text]");
	else
	{
	    GetPlayerName(playerid,name,sizeof(name));
	    if(PlayerInfo[playerid][pAdmin] == 1) { arank = "Admin"; }
		else if(PlayerInfo[playerid][pAdmin] == 2) { arank = "Napredni Admin"; }
		else if(PlayerInfo[playerid][pAdmin] == 3) { arank = "Glavni Admin"; }
		else if(PlayerInfo[playerid][pAdmin] == 4) { arank = "Vlasnik"; }
		else if(PlayerInfo[playerid][pAdmin] == 5) { arank = "Skripter"; }
...
YCMD:jail(playerid, params[], help) 
{
  ...
  if(sscanf(params,"is[50]", id, razlog)) return SendClientMessage(playerid, SIVA, "Koristi: /jail [id igraca] [razlog]");
  ...
  va_SendClientMessage(playerid, -1, "Zatvorili ste igraca [ID:%d] %s na 60 minuta | Razlog: %s", id, GetName(id), razlog);
  va_SendClientMessageToAll(RED, "[A-JAIL] Igrac [ID:%d] %s je zatvoren na 60 minuta od strane admina | Razlog: %s", id, GetName(id), razlog);
...
YCMD:setstats(playerid, params[], help)
{
	if(IPC(playerid))
	{
...
////////////////////////////////////////////////////////////////////////////////
YCMD:skiniopremu(playerid, params[], help)
{
	new string[50];
...
////////////////////////////////////////////////////////////////////////////////
YCMD:deminiraj(playerid, params[], help)
{
...
////////////////////////////////////////////////////////////////////////////////
YCMD:izvadiminu(playerid, params[], help)
{
    new string[100];
	if(PlayerInfo[playerid][pPosao] != 2) return SCM(playerid, SIVA," Morate biti zaposleni kao deminer !");
	if(Oprema[playerid] == 0) return SCM(playerid, SIVA," Morate obuci opremu kako bi bili zasticeni !!");
...
YCMD:dveh(playerid, cmdtext[], help)
{
    new vozilo;
	if(PlayerInfo[playerid][pAdmin] < 2) return SCM(playerid,SIVA,"[UB Administracija] Niste ovlasceni da koristite ovu komandu");
...
YCMD:oruzije1(playerid, params[], help) 
{
...
YCMD:gotobg(playerid, params[], help)
{
	if(PlayerInfo[playerid][pAdmin] == 0) return SCM(playerid, SIVA," Niste ovlasceni da koristite ovu komandu");
  if(PlayerInfo[playerid][Zatvoren] > 0) return SendClientMessage(playerid, SIVA, "Ne mozete to dok ste zatvoreni");
...
YCMD:gotoautosk(playerid, params[], help)
{
    if(PlayerInfo[playerid][pAdmin] == 0) return SCM(playerid, SIVA," Niste ovlasceni da koristite ovu komandu");
    if(PlayerInfo[playerid][Zatvoren] > 0) return SendClientMessage(playerid, SIVA, "Ne mozete to dok ste zatvoreni");
    if(GetPlayerInterior(playerid) > 0) return SCM(playerid, SIVA," Ne mozete se teleportovati dok ste u enterijeru!");
...
YCMD:gotoab(playerid, params[], help)
{
	if(IPC(playerid))
	{
    if(PlayerInfo[playerid][pAdmin] == 0) return SCM(playerid, SIVA," Niste ovlasceni da koristite ovu komandu");
...
YCMD:dostavilijekove(playerid, params[], help)
{
...
YCMD:gotopd(playerid, params[], help)
{
	if(PlayerInfo[playerid][pAdmin] < 1) return SCM(playerid, SIVA," Niste ovlasceni da koristite ovu komandu");
  if(GetPlayerInterior(playerid) > 0) return SCM(playerid, SIVA,"Ne mozete se teleportovati dok ste u enterijeru");
  if(PlayerInfo[playerid][Zatvoren] > 0) return SendClientMessage(playerid, SIVA, "Ne mozete to dok ste zatvoreni");
...
YCMD:uplatipolaganje(playerid, params[], help)
{
	if(IPC(playerid))
	{
...
YCMD:ispit(playerid, params[], help)
{
	if(IPC(playerid))
	{
...
YCMD:izvadidozvolu(playerid, params[], help)
{
...
YCMD:dutytest(playerid, params[], help) 
{
  new string[100];
  if(Duty[playerid] == 0)
  {
...
YCMD:dajwl(playerid, params[], help)
{
	SCM(playerid, SIVA," WL RADI");
	SetPlayerWL(playerid, 1, "Test");
	return 1;
}
YCMD:skiniwl(playerid, params[], help)
{
